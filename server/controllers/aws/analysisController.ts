    ...
    const vals = tidy(
      ...
      groupBy(graphUnits, [
        summarize({
          ...
          total_pct_memory_used: sum(
            (d) =>
              (Number(d['Max Memory Used' as keyof typeof d]) /
                Number(d['Memory Size' as keyof typeof d])) *
              100
          ),
          avg_pct_memory_used: mean(
            (d) =>
              (Number(d['Max Memory Used' as keyof typeof d]) /
                Number(d['Memory Size' as keyof typeof d])) *
              100
          ),
          total_memory_overallocated: sum(
            (d) =>
              Number(d['Memory Size' as keyof typeof d]) -
              Number(d['Max Memory Used' as keyof typeof d])
          ),
...
analysisController.calcMemoryUsage = async (req, res, next) => {
  ...
  try {
    ...
    for (let i = 0; i < summarizedData.length; i++) {
      ...
      for (let j = 0; j < summarizedData[i].streams.length; j++) {
        ...
        formattedOutput[j][`${summarizedData[i].name}`] =
          summarizedData[i].streams[j].memory_usage;
      }
    }
...
analysisController.calcMeanMemoryUsageTotal = async (req, res, next) => {
  const { logs, funcNames } = res.locals;
  const periodSelection = req.params.period;
  let [graphPeriod, graphUnits] =
    formatController.periodDefinitions[periodSelection];
...
analysisController.calcLambdaMemoryDiff = async (req, res, next) => {
  const { logs, funcNames } = res.locals;
  const periodSelection = req.params.period;
  let [graphPeriod, graphUnits] =
    formatController.periodDefinitions[periodSelection];
  ...
  try {
    for (let func of logs) {
      const summary = tidy(
        func.streams,
        summarize({
          [`used${func.name}`]: mean((d) =>
            Number(d['Max Memory Used' as keyof typeof d])
          ),
          [`allo${func.name}`]: mean((d) =>
            Number(d['Memory Size' as keyof typeof d])
          ),
          [`diff${func.name}`]: mean(
            (d) =>
